// assets/js/main.js

$(document).ready(function() {
  // 1. Friendly cookie-based greeting
  function friendlyGreeting() {
    const lastCookie = document.cookie
      .split('; ')
      .find(row => row.startsWith('lastVisit='));
    const lastVisit = lastCookie && lastCookie.split('=')[1];
    if (!lastVisit) {
      alert('Welcome to my homepage for the first time!');
    } else {
      alert(`Welcome back! Your last visit was ${decodeURIComponent(lastVisit)}`);
    }
    // update cookie
    document.cookie = `lastVisit=${encodeURIComponent(new Date())};path=/;max-age=${60*60*24*365}`;
  }
  friendlyGreeting();

  // 2. Show/hide email
  $('#toggle-email').click(function() {
    $('#email').toggle();
    $(this).text(function(_, t) {
      return t === 'Show Email' ? 'Hide Email' : 'Show Email';
    });
  });

  // 3. Digital clock
  function updateDigitalClock() {
    $('#digital-clock').text(new Date().toLocaleTimeString());
  }
  setInterval(updateDigitalClock, 500);
  updateDigitalClock(); // init immediately

  // 4. Analog clock (simple example)
  const ctx = document.getElementById('analog-clock').getContext('2d');
  function drawAnalogClock() {
    const radius = ctx.canvas.width / 2;
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ctx.save();
    ctx.translate(radius, radius);
    // draw clock face
    ctx.beginPath();
    ctx.arc(0, 0, radius * 0.9, 0, 2 * Math.PI);
    ctx.fillStyle = '#fff';
    ctx.fill();
    ctx.lineWidth = radius * 0.05;
    ctx.stroke();
    // draw hands
    const now = new Date();
    let ang, len;
    // hour
    ang = ((now.getHours() % 12) + now.getMinutes() / 60) * Math.PI/6;
    len = radius * 0.5;
    drawHand(ang, len, radius * 0.07);
    // minute
    ang = (now.getMinutes() + now.getSeconds()/60) * Math.PI/30;
    len = radius * 0.8;
    drawHand(ang, len, radius * 0.05);
    // second
    ang = now.getSeconds() * Math.PI/30;
    len = radius * 0.9;
    drawHand(ang, len, radius * 0.02, 'red');
    ctx.restore();
  }
  function drawHand(angle, length, width, color='#000') {
    ctx.beginPath();
    ctx.lineWidth = width;
    ctx.lineCap = 'round';
    ctx.strokeStyle = color;
    ctx.moveTo(0,0);
    ctx.rotate(angle);
    ctx.lineTo(0, -length);
    ctx.stroke();
    ctx.rotate(-angle);
  }
  setInterval(drawAnalogClock, 1000);
  drawAnalogClock();

  // 5. Moment.js date display
  $('#today').text('Today is ' + moment().format('dddd, MMMM Do YYYY'));

  // 6. JokeAPI integration
  async function fetchJoke() {
    try {
      const res = await fetch('https://v2.jokeapi.dev/joke/Any?type=single');
      const data = await res.json();
      $('#joke-text').text(data.joke);
    } catch {
      $('#joke-text').text('Failed to load joke.');
    }
  }
  fetchJoke();
  setInterval(fetchJoke, 60000);

  // 7. XKCD comic integration
  async function fetchComic() {
    try {
      const res = await fetch('https://xkcd.com/info.0.json');
      const { img, title } = await res.json();
      $('#comic-container').html(
        `<img src="${img}" alt="${title}" class="img-fluid"><p>${title}</p>`
      );
    } catch {
      $('#comic-container').text('Failed to load comic.');
    }
  }
  fetchComic();
});

